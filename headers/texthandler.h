...
std::vector<std::string> EvaluationTexts_short{
...
std::vector<std::string> EvaluationTexts_medium{
...
std::vector<std::string> EvaluationTexts_long{
...
void LoadParasfromText (const char* filename) {
    ...
    if (fileHandle.is_open()) {
        ...
        while (std::getline(fileHandle, para)) {
            if (para.length() <= 200) {
                EvaluationTexts_short.push_back(para);
            }
            else if (para.length() <= 400) {
                EvaluationTexts_medium.push_back(para);
            }
            else {
                EvaluationTexts_long.push_back(para);
            }
        }
        fileHandle.close();
